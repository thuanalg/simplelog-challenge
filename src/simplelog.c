...
#define SPL_FCLOSE(__fp__, __n)                                                                                             \
	{                                                                                                                   \
		if (__fp__) {                                                                                               \
			...
			if (__n) {                                                                                          \
				spl_console_log("Close FILE error.");                                                       \
...
#define SPLOG_PROCESS_MODE "process_mode="
#define SPLOG_SHARED_KEY "shared_key="
...
spl_del_memory()
{
	...
	do {
		...
		if (t->is_master) {
			...
			if (ret) {
				spl_console_log("pthread_spin_destroy/mtx_rw: err: %d, errno: %d, text: %s.", ret, errno,
				    strerror(errno));
			}
			for (i = 0; i < t->ncpu; ++i) {
				ret = pthread_spin_destroy((pthread_spinlock_t *)t->arr_mtx[i]);
				if (ret) {
					spl_console_log("pthread_spin_destroy/arr_mtx: err: %d, errno: %d, text: %s.", ret,
			...
			if (ret) {
				spl_console_log("pthread_mutex_destroy/mtx_rw: err: %d, errno: %d, text: %s.", ret, errno,
				    strerror(errno));
			}
			for (i = 0; i < t->ncpu; ++i) {
				ret = pthread_mutex_destroy((pthread_mutex_t *)t->arr_mtx[i]);
				if (ret) {
					spl_console_log("pthread_mutex_destroy/arr_mtx: err: %d, errno: %d, text: %s.", ret,
					    errno, strerror(errno));
				}
			}
#endif
	...
	do {
		...
		if (t->isProcessMode) {
			if (t->is_master) {
				...
				if (err) {
					spl_console_log("ftruncate, errno: %d, errno_text: %s, shared_key: %s.", errno,
		...
		/*
			err = ftruncate(hMapFile, size_shared);
			if (err) {
		    spl_console_log("ftruncate, errno: %d, errno_text: %s, shared_key: %s.", errno, strerror(errno),
		   shared_key); ret = SPL_LOG_SHM_UNIX_TRUNC; break;
			}
		*/
		...
		if (p == MAP_FAILED || p == 0) {
			...
			spl_console_log("mmap, errno: %d, errno_text: %s, hMapFile: %d, size_shared: %d.", errno,
